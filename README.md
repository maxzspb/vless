...
`Table = off` + PostUp создаёт отдельную таблицу маршрутов 2408. Xray использует `sendThrough: 172.16.0.2` — ядро смотрит в таблицу 2408 и отправляет через warp-интерфейс. Контейнеру не нужны права CAP_NET_RAW.
...
Если нужно настроить вручную — **Xray Configs → Outbounds → Add Outbound**.

> ⚠️ Кнопку **WARP** не нажимай — Cloudflare API недоступен со многих VPS. Добавь outbound через JSON-редактор:
...
Добавь ещё два: `freedom` → tag `direct`, `blackhole` → tag `blocked`.

> ⚠️ Не используй вкладку **Advanced / Xray Template Config** — JSON с inbound там создаст дубль и Xray упадёт с `exit status 2`.
...
> ✅ Скрипт создаёт inbound автоматически. Этот раздел — для ручной настройки или пересоздания.
...
> ⚠️ XHTTP — Xray-специфичная фича. Используй клиенты на базе Xray-core, не v2ray-core.
