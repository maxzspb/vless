...
- [Ручная установка Общее](#ручная-установка-общее)
- [Ручная установка Hysteria2](#ручная-установка-hysteria2)
- [Ручная установка Vless](#ручная-установка-vless)
- [Настройка VLESS в панели](#настройка-vless-в-панели)
- [Клиент](#клиент-вариантов-больше)
- [Проверка](#проверка)
- [Улучшения](#улучшения)
...
- Открытые порты в панели хостера: `443/tcp`, `443/udp`, `8443/udp`, `20000-50000/udp`
...
Потребуется ручная одна ручная манипуляция в панеле 3x-ui после скрипта, в остальном готовые конфиги
---
## Ручная установка Общее

### 1. Базовые пакеты и Docker

```bash
apt update && apt upgrade -y
apt install -y curl wget git wireguard netfilter-persistent iptables-persistent sqlite3
...
`Table = off` + PostUp создаёт отдельную таблицу маршрутов 2408. Xray использует `sendThrough: 172.16.0.2` — ядро смотрит в таблицу 2408 и отправляет через warp-интерфейс. 
...
```bash
cd ~/vless && docker compose -f docker-compose.hysteria.yaml up -d
docker ps
```

## Ручная установка Vless
...
```bash
cd ~/vless && docker compose -f docker-compose.xui.yaml up -d
docker ps
```

### 4. Автообновление РУ геоблока
...
```bash
ssh -L 2053:127.0.0.1:2053 user@<IP_VPS> -p <SSH_PORT>
```
...
> ⚠️ Кнопка **WARP** не всегда срабатывает. Если гуд то Xray автоматически настроит, если нет добавь outbound который мы ранее в терминале настроили через JSON-редактор:
...
Должно получиться три outbound как ниже:

| # | Tag | Protocol |
|---|-----|----------|
| 1 | warp | wireguard → engage.cloudflareclient.com:2408 |
| 2 | direct | freedom |
| 3 | blocked | blackhole |

Нажимаем **Save** → **Restart Xray**.
...
**Sniffing:** включить HTTP + TLS + QUIC + FAKEDNS. **Save.**
...
**Проблема:** `www.microsoft.com` находится в другой подсети. DPI видит несовпадение IP назначения и SNI.

**Решение:** найти домен из той же ASN что и VPS.
...
cat > /usr/local/etc/xray/config.json << 'EOF'
{
  "inbounds": [{
    ...
    "settings": { "address": "<IP_ЕВРОПЕЙСКОГО_VPS>", "port": 443, "network": "tcp,udp" }
  }],
...
**Соединение молча сбрасывается**
→ Проверь Authentication в inbound — должно быть пусто, не ML-KEM-768. `decryption` и `encryption` должны быть `"none"`.
...
**Xray exit status 2 — дубль inbound**

Причина: в `xrayTemplateConfig` сохранился inbound — 3x-ui добавляет его поверх записи из таблицы `inbounds`.
```bash
docker stop 3x-ui
...
```
Не используй вкладку Advanced/Template в панели.

**WARP button → TLS handshake timeout**
→ Cloudflare API заблокирован. Используй `sendThrough: 172.16.0.2` в outbound JSON вручную.
...
```bash
ssh -L 2053:127.0.0.1:2053 user@<IP_VPS> -p <SSH_PORT> -N &
...
```bash
docker stop 3x-ui
sqlite3 ~/vless/db/x-ui.db "UPDATE users SET username='admin', password='admin' WHERE id=1;"
docker start 3x-ui
```
